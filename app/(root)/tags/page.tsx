    ...
    <>
      ...
      <section className='mt-12 flex flex-wrap gap-4'>
        {result.tags.length > 0 ? (
          result.tags.map((tag) => (
            <Link href={`/tags/${tag._id}`} className='shadow-light100_darknone' key={tag._id}>
              <article className='background-light900_dark200 light-border flex w-full flex-col items-center rounded-2xl border px-8 py-10 sm:w-[260px]'>
                <div className='background-light800_dark400 mb-1 w-fit rounded-sm px-5 py-1.5'>
          ...
          <NoResult title='No Tags Found' description='It looks like there are no tags yet' link='/ask-question' linkTitle='Ask a question' />
        )}
      </section>
    </>
  );
